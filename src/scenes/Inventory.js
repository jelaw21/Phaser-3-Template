...
export default class Inventory extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(510, 72, this.player.inventory[0].name + ":  " + this.player.inventory[0].quantity, {fontSize: '24px', fontFamily: 'Marmelad', fill: '#ffffff'});
        ...
        for (let i = 1; i < this.player.inventory.length; i++) {
            if (this.player.inventory[i].name === 'HAIR') {
                ...
                let invItem = this.add.sprite(0, 0, this.player.inventory[i].icon).setInteractive().setOrigin(0.5).setSize(24,24).setName(this.player.inventory[i].name);
    ...
    showDescr(pointer, invItem){
        ...
        for(let i = 0; i < this.player.inventory.length; i++){
            ...
            if(item.name === invItem.name){
                this.itemNameText.setText(item.name).setVisible(true);
                this.itemDescText.setText(item.desc).setVisible(true);
                this.itemArmPerc.setText("ARMOR PROTECTION %: " + item.effect).setVisible(true);
                this.itemArmMax.setText("MAX PROTECTION PTS: " + item.maxEffect).setVisible(true);
            }
        }
    }
